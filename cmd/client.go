...
func init() {
	...
	sendMail.Flags().StringVarP(&sender, "sender", "s", "", "email address of sender")
...
var sendMail = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err := smtp.SendEmail(serverAddress, sender, recipient, message)
		if err != nil {
		...
		if len(args) > 0 {
			...
			for _, email := range emails {
				...
				if len(email.From) > 30 {
					fmt.Printf("|%-20s..", email.From[0:28])
				} else {
					fmt.Printf("|%-20s", email.From)
				}
...
func printDivider() {
	...
	fmt.Printf("+%-29s", strings.Repeat("-", 29))
